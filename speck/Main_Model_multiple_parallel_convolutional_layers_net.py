...
from tensorflow.keras.layers import Dense, AveragePooling1D, Conv1D, MaxPooling1D, Input, Reshape, Permute, Add, \
...
def make_resnet(pairs=2, num_blocks=3, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=16, ks=3, depth=5,
                reg_param=0.0001, final_activation='sigmoid'):
    inp = Input(shape=(2 * num_blocks * pairs * word_size,))

    # --- 核心修正区域 START ---

    # 目的：构造 (Batch, Steps=16, Channels=2*pairs) 的形状
    # 这样卷积核 (Size=3) 才能在 16 个比特上滑动，模拟循环移位

    # 1. 先把 word_size 放在最后: (Batch, Total_Channels, Word_Size)
    rs = Reshape((2 * num_blocks * pairs, word_size))(inp)

    # 2. 交换维度: (Batch, Word_Size, Total_Channels)
    # 现在的 Steps = 16 (足够容纳 Kernel=3)
    perm = Permute((2, 1))(rs)
...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1, pairs=1):
    ...
    check = make_checkpoint(
        wdir + 'best' + str(num_rounds) + 'r_depth' + str(depth) + "_num_epochs" + str(num_epochs) + "_pairs" + str(
            pairs) + '.h5')
    # create learnrate schedule
    lr = LearningRateScheduler(cyclic_lr(5, 0.002, 0.0001))
    # train and evaluate
