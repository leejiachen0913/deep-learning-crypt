...
# 加载错误密钥的反馈结果值，均值和方差，用于计算后续的参考值（参考论文
# 所用的结果来自compute.wrong_key_response.py
...
# 从初始差分0x8020加密若干轮次得到0040，扩展密钥恢复的轮次
def make_structure(pt0, pt1, diff=(0x8020, 0x4101),
...
def gen_challenge(pt0, pt1, key, diff=(0x8020, 0x4101), neutral_bits=[20, 21, 22, 14, 15, 23]):
    ...
    # 把这几个数据复制1份
    ct0a = ct0a.reshape(pairs, len(pt0a) // pairs, -1);
    ct1a = ct1a.reshape(pairs, len(pt0a) // pairs, -1);
    ct0b = ct0b.reshape(pairs, len(pt0a) // pairs, -1);
    ct1b = ct1b.reshape(pairs, len(pt0a) // pairs, -1);
...
def test_bayes(device, cts, pairs, it=1, cutoff1=10, cutoff2=10, net=net8, net_help=net7, m_main=m8, m_help=m7,
               s_main=s8, s_help=s7):
    ...
    for j in range(it):
        ...
        keys, scores, v = bayesian_key_recovery(
            pairs, [cts[0][i::n], cts[1][i::n], cts[2][i::n], cts[3][i::n]], num_cand=32, num_iter=5, net=net, m=m_main,
            s=s_main)
...
def test(device, n=20, pairs=8, nr=13, num_structures=2 ** 11, it=2 ** 13, cutoff1=25, cutoff2=-650,
         neutral_bits=[[22], [14, 21], [6, 29], [30], [0, 8, 31], [5, 28], [15, 24], [4, 27, 29]], net=net8,
         net_help=net7, m_main=m8, s_main=s8, m_help=m7, s_help=s7):
    ...
    for i in range(n):
        ...
        guess, num_used = test_bayes(device, ct, pairs=pairs, it=it, cutoff1=cutoff1, cutoff2=cutoff2, net=net,
                                     net_help=net_help,
