...
from tensorflow.keras.layers import Dense, Conv1D, GlobalAveragePooling2D, GlobalAveragePooling1D, MaxPooling2D, Input, \
    Reshape, Permute, Add, Flatten, BatchNormalization, Activation
...
def TeacherModel(pairs=2, num_blocks=3, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=16, ks=3, depth=5,
                 reg_param=0.0001):
...
def StudentModel(pairs=2, num_blocks=3, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=16, ks=3, depth=5,
...
def teacher_distinguisher_train(num_rounds, num_epochs, pairs, depth, batch_size):
    ...
    X_eval, Y_eval = sp.make_train_data(10 ** 5, num_rounds, pairs=pairs);
    ...
    h = model.fit(X, Y, epochs=num_epochs, batch_size=batch_size, validation_data=(X_eval, Y_eval),
...
def student_distinguisher_knowledge_distillation(Teacher_fn, num_rounds, num_epochs, pairs, depth, batch_size):
    ...
    X_eval, _ = sp.make_train_data(10 ** 5, num_rounds, pairs=pairs);
    strategy = tf.distribute.MirroredStrategy()
...
if __name__ == "__main__":
    ...
    # 训练蒸馏Student模型
    file_name = './teacher_distinguisher/' + 'speck_model_6r_num_epochs20_acc_0.8667299747467041.h5'
    for r in rounds:
        student_distinguisher_knowledge_distillation(Teacher_fn=file_name, num_rounds=r, num_epochs=40, pairs=pairs,
                                                     depth=5, batch_size=batch_size)
